...
test('temporary routine cleaner keeps latest gym block and resets old progression', async ({ page }) => {
  await page.setViewportSize({ width: 320, height: 568 });
  await page.addInitScript(() => {
    localStorage.setItem('liftday_onboarding_completed', 'true');
    localStorage.setItem('traindaily_sessions', JSON.stringify({
      '2026-05-08': {
        logged_at: '2026-05-08T05:15:09.063Z',
        week_number: 10,
        workout_type: 'pull',
        lat_pulldown: [{ reps: 10, weight: 40, rir: 2 }],
      },
      '2026-05-11': {
        logged_at: '2026-05-11T05:15:49.348Z',
        week_number: 11,
        workout_type: 'push_a',
        smith_incline_press: [{ reps: 8, weight: 50, rir: 2 }],
      },
      '2026-05-19': {
        logged_at: '2026-05-19T05:26:24.394Z',
        week_number: 12,
        workout_type: 'pull_a',
        lat_pulldown: [{ reps: 10, weight: 45, rir: 2 }],
      },
    }));
    localStorage.setItem('traindaily_first_session', '2026-02-24');
    localStorage.setItem('liftday_active_workout_draft', JSON.stringify({
      version: 1,
      dateKey: '2026-05-19',
      workoutType: 'pull_a',
      savedAt: '2026-05-19T05:10:00.000Z',
    }));
    localStorage.setItem('liftday_daily_logs', JSON.stringify({
      '2026-05-19': { dateKey: '2026-05-19', morningWeightKg: 68.7 },
    }));
    localStorage.setItem('liftday_user_profile', JSON.stringify({
      activeRoutine: 'gym',
      tiers: { gym_pull_vertical: 2 },
      tierProgress: {
        gym_pull_vertical: {
      ...
      createdAt: '2026-04-18T06:28:02.378Z',
      setsPerExercise: 3,
      heightCm: 172,
      weightKg: 66.6,
  ...
  await page.getByRole('button', { name: /clean routine/i }).click();
  await page.getByRole('button', { name: /^clean$/i }).click();

  await expect(page.getByRole('button', { name: /kept 2\. removed 1\./i })).toBeVisible();
  await expect.poll(() => page.evaluate(() => {
    const sessions = JSON.parse(localStorage.getItem('traindaily_sessions') ?? '{}') as Record<string, unknown>;
    const profile = JSON.parse(localStorage.getItem('liftday_user_profile') ?? '{}') as {
      tiers?: Record<string, unknown>;
      tierProgress?: Record<string, unknown>;
      weightKg?: number;
      waistCircumferenceCm?: number;
    };
    return {
      sessionDates: Object.keys(sessions).sort(),
      firstSessionDate: localStorage.getItem('traindaily_first_session'),
      activeDraft: localStorage.getItem('liftday_active_workout_draft'),
      dailyLogs: localStorage.getItem('liftday_daily_logs'),
      profileWeight: profile.weightKg,
      profileWaist: profile.waistCircumferenceCm,
      tierCount: Object.keys(profile.tiers ?? {}).length,
      tierProgressCount: Object.keys(profile.tierProgress ?? {}).length,
      backupKeys: Object.keys(localStorage).filter((key) => key.startsWith('liftday_temp_routine_cleanup_backup_')).length,
    };
  })).toEqual({
    sessionDates: ['2026-05-11', '2026-05-19'],
    firstSessionDate: '2026-05-11',
    activeDraft: null,
    dailyLogs: JSON.stringify({ '2026-05-19': { dateKey: '2026-05-19', morningWeightKg: 68.7 } }),
    profileWeight: 66.6,
    profileWaist: 76.5,
    tierCount: 0,
    tierProgressCount: 0,
